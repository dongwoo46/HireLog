...
/**
 * ============================================================
 * RECOVERY LOAD TEST (Resilience Verification)
 * ------------------------------------------------------------
 * 목적:
 * - Consumer 장애 시 Kafka backlog 누적 확인
 * - Consumer 재기동 후 backlog 회복 여부 확인
 * - 데이터 유실 여부 검증
 *
 * 이 테스트는 성능 측정이 아니라
 * "복원력(Resilience)" 검증이 목적이다.
 *
 * 단계:
 * 1) 정상 구간 (1분)
 * 2) Consumer kill (1분)
 * 3) Consumer 재기동 후 회복 구간 (2분)
 * ============================================================
 */
...
export const options = {
  scenarios: {
    recovery: {
      ...
      vus: 50, // baseline보다 약간 높은 부하
      duration: '2m', // 정상 → 장애 → 복구 구간 확보
    },
  },
};
...
export function setup() {
  ...
  console.log('===== RECOVERY TEST START =====');
  console.log(`Test Start Time: ${start}`);
  console.log('▶ 60초 후 Python consumer 수동 kill');
  console.log('▶ 120초 후 다시 기동');
  console.log('▶ Lag 피크 및 회복 시간 측정');

  return { testStart: start };
}

// ---------- Test Logic ----------
export default function () {
  const payload = JSON.stringify({
    brandName: 'RecoveryBrand',
    brandPositionName: 'Backend Developer',
    jdText:
      `RECOVERY_${__VU}_${__ITER}` +
 ...
 * ============================================================
 *
 * 1) Consumer 중단 구간에서 Kafka lag 증가량
 * 2) Consumer 재기동 후 lag 감소 속도
 * 3) Lag 0까지 회복 시간
 * 4) DB JobSummary count vs OpenSearch count
 * 5) Outbox 잔여 이벤트 여부
 *
 * 성공 조건:
 * - HTTP 실패 없음
