 ...
 * ------------------------------------------------------------
 * 목적:
 * - 이벤트 드리븐 파이프라인의 정상 동작 구간 검증
 *
 * 검증 대상:
 * HTTP → DB → Outbox → Debezium → Kafka → Consumer → OpenSearch
 *
 * 성공 기준:
 * - 실패율 < 1%
 * - p95 < 1500ms
 * - Kafka Lag 0 유지
 * - DB count == OpenSearch count
 * ============================================================
 */
...
export const options = {
  scenarios: {
    baseline: {
      ...
      vus: 30, // 현실적인 동시 사용자
      duration: '1m', // 안정 구간 확인용
    },
  },
  thresholds: {
    http_failure_rate: ['rate<0.01'],
    http_latency_ms: ['p(95)<1500'],
  },
};
...
export function setup() {
  ...
  console.log(`BASELINE START: ${start}`);
