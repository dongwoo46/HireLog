...
- [ ] 리뷰 관리/리뷰 목록 조회 불가 원인 파악 및 수정
- [ ] 상세페이지 준비기록 저장 실패 수정 (서류전형, 코딩테스트)
- [ ] 관리자 채용공고 상태 전환 버그 수정 (비활성 -> 활성 불가)
- [ ] 내가 저장한 채용공고 목록 조회 불가 수정
- [ ] 알림 조회 API 동작/응답 스키마 점검 및 수정

## 관리자 분리

- [ ] JD 요약 요청 관리자 전용 API 분리
- [ ] 관리자 권한 검증 미들웨어/가드 점검
