 ...
 * RAG 서비스 오케스트레이터
 *
 * 흐름: RateLimiter → Parser → Executor → Composer → QueryLog 저장
 *
 * 책임:
 * - 일일 호출 제한 검증 (USER: 3회, ADMIN: 무제한)
 * - Parser / Executor / Composer 순차 조합
 * - 질의 로그 저장 (실패해도 응답에 영향 없음)
...
class RagService(
    ...
    private val ragQueryLogCommand: RagQueryLogCommand,
    ...
    fun query(question: String, memberId: Long, isAdmin: Boolean): RagAnswer {
        ...
        saveQueryLog(memberId, question, ragQuery, context, answer)

        log.info("[RAG_QUERY_DONE] memberId={}, intent={}", memberId, ragQuery.intent)
        return answer
    }

    private fun saveQueryLog(
        memberId: Long,
        question: String,
        ragQuery: RagQuery,
        context: RagContext,
        ...
        runCatching {
            ragQueryLogCommand.save(
                RagQueryLog.create(
                    memberId = memberId,
                    question = question,
                    intent = ragQuery.intent,
                    parsedText = ragQuery.parsedText,
                    parsedFiltersJson = objectMapper.writeValueAsString(ragQuery.filters),
                    contextJson = objectMapper.writeValueAsString(context),
                    answer = answer.answer,
                    reasoning = answer.reasoning,
                    evidencesJson = answer.evidences?.let { objectMapper.writeValueAsString(it) },
                    sourcesJson = answer.sources?.let { objectMapper.writeValueAsString(it) }
                )
            )
        }.onFailure { e ->
            log.error("[RAG_LOG_SAVE_FAILED] memberId={}, error={}", memberId, e.message)
        }
    }
}
