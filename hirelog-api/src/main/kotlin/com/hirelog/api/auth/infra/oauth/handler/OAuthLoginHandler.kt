...
import com.hirelog.api.common.exception.SuspendedMemberException
    ...
    override fun loginProcess(oAuthUser: OAuthUser): OAuth2LoginResult {
        ...
        if (existingAccount != null) {
            ...
            return when (member.status) {
                MemberStatus.ACTIVE ->
                    OAuth2LoginResult.ExistingUser(member.id, member.role.name)

                MemberStatus.DELETED ->
                    OAuth2LoginResult.RecoverableUser(
                        memberId = member.id,
                        provider = oAuthUser.provider,
                        providerUserId = oAuthUser.providerUserId,
                ...
                MemberStatus.SUSPENDED ->
                    throw SuspendedMemberException()
            }
        }
