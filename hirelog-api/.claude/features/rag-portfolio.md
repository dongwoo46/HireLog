...
유저가 자연어로 질문하면 자신의 저장 공고·면접 기록·전체 공고 데이터를 기반으로 정확한 답변을 생성한다.

**예시 질문:**
- "내가 저장한 핀테크 공고에서 가장 많이 요구하는 기술스택은?"
- "Kafka 쓰는 백엔드 공고 찾아줘"
- "1차 면접 불합격 공고들의 공통점이 뭐야?"
- "합격한 공고가 전체 평균 대비 어떤 기술을 더 요구해?"
...
[RagService] ← RateLimiter (Redis, USER 3회/일)
    ↓
[RagLlmParser Port] ← GeminiRagParserAdapter
    ↓ RagQuery { intent, filters, semanticRetrieval, ... }
[RagQueryExecutor]
    ├── DOCUMENT_SEARCH / SUMMARY
    │     ↓ RagEmbedding Port → Python 임베딩 서버
    ...
    │
    └── EXPERIENCE_ANALYSIS
          ↓ RagCohortQuery Port → PostgreSQL (HiringStageRecord)
    ↓ RagContext (구조화된 결과)
[RagLlmComposer Port] ← GeminiRagComposerAdapter
...
| `EXPERIENCE_ANALYSIS` | 면접/전형 경험 분석 | DB HiringStageRecord | 2회 |
...
cohort 문서에서 정성적 특징("대용량 트래픽", "MSA / 이벤트 기반" 등)을 추출할 때  
LLM과 시스템의 역할을 명확히 분리했다.

| 역할 | 담당 |
|---|---|
| 레이블 추출 ("대용량 트래픽") | Gemini (LLM) |
...
LLM은 "무엇이 특징인가"만 판단하고, 검증 가능한 수치는 시스템이 계산한다.
...
필드명 concat: "responsibilities: ... requiredQualifications: ..."
...
raw 수치·ID 계산은 Executor 단계에서 완료되므로 Composer는 해석만 담당한다.
...
### KEYWORD_SEARCH intent를 제거한 이유

초기 설계에서 키워드 검색 전용 intent를 두려 했으나,  
`DOCUMENT_SEARCH`의 Hybrid(k-NN+BM25)가 이미 BM25를 포함하므로 불필요하다.  
Parser fallback도 `DOCUMENT_SEARCH`로 통합해 intent 수를 줄였다.
...
│   │   └── RagAnswer.kt                     ← 최종 응답 (answer, reasoning, evidences, sources)
│   ├── port/
│   │   ├── RagLlmParser.kt                  ← 질문 → RagQuery
│   │   ├── RagLlmComposer.kt                ← RagContext → RagAnswer (+ RagContext, RagDocument 등 정의)
...
└── presentation/controller/
    └── RagController.kt                     ← POST /api/rag/query
...
- LLM hallucination 방지: Feature Extractor는 레이블만 추출, count/snippet/ID는 시스템이 직접 계산
